...
class WorkInterface(StillInterface):
    ...
    def __init__(self, objectName: str, title: str, imageColorizer: ModelImageVisualizer, simpleLama: SimpleLama, maskGenerator: SamPredictor, yolov7Detector: Yolov7Detector, parent=None):
        super().__init__(objectName, title, parent)

        self.imageWidget: DraggableImage = DraggableImage(
            imageColorizer, simpleLama, maskGenerator, yolov7Detector, self)
        self.imageWidget.setZoomFactorLabelTextSignal.connect(
            self.setZoomFactorLabelText)
        self.imageWidget.setUndoBtnEnabledSignal.connect(
            self.setUndoBtnEnabled)
        self.imageWidget.setRedoBtnEnabledSignal.connect(
            self.setRedoBtnEnabled)
        self.imageWidget.setSpinnerEnabledSignal.connect(
            self.setSpinnerEnabledSignal.emit)
        self.imageWidget.setColorLabelTextSignal.connect(
            self.setColorLabelText)

        self.bottomLeftCmdBar: QWidget = self.createBottomLeftCmdBar()

        self.pageLayout.addWidget(
            self.createTopLeftCmdBar(), 0, 0, 1, 1, alignment=Qt.AlignLeft)
        self.pageLayout.addWidget(
            self.createTopMiddleCmdBar(), 0, 1, 1, 3, alignment=Qt.AlignHCenter)
        self.pageLayout.addWidget(
            self.createTopRightCmdBar(), 0, 4, 1, 1, alignment=Qt.AlignRight)
        self.pageLayout.addWidget(self.imageWidget, 1, 0, 1, 5)
        self.pageLayout.addWidget(
            self.bottomLeftCmdBar, 2, 0, 1, 4, alignment=Qt.AlignLeft)
        self.pageLayout.addWidget(
            self.createBottomRightCmdBar(), 2, 4, 1, 1, alignment=Qt.AlignRight)
    ...
    def createTopRightCmdBar(self) -> QWidget:
        ...
        def onConfigCmdClicked():
            ...
            penWidthFactorSpinBox.valueChanged.connect(
                penWidthFactorSpinBoxValueChanged)
    ...
    def createTopLeftCmdBar(self) -> QWidget:
        ...
        pointSelectCommand: TransparentToggleToolButton = TransparentToggleToolButton(
            widget)
        pointSelectCommand.setIcon(FluentIconExt.GRID_DOTS)
        pointSelectCommand.clicked.connect(lambda: self.onCommandClicked(
            Command(CommandMode.POINT_SELECT_REMOVE, True), widget))
        Util.setModernTooltipText(pointSelectCommand, pointSelectCommandLabel)

        rectSelectCommandLabel: str = '框选消除'
        rectSelectCommand: TransparentToggleToolButton = TransparentToggleToolButton(
            widget)
        rectSelectCommand.setIcon(FluentIconExt.SELECT_OBJECT)
        rectSelectCommand.clicked.connect(lambda: self.onCommandClicked(
            Command(CommandMode.RECTANGLE_SELECT_REMOVE, True), widget))
        Util.setModernTooltipText(rectSelectCommand, rectSelectCommandLabel)

        paintSelectCommandLabel: str = '涂抹消除'
        paintSelectCommand: TransparentToggleToolButton = TransparentToggleToolButton(
            widget)
        paintSelectCommand.setIcon(FluentIconExt.HAND_DRAW)
        paintSelectCommand.clicked.connect(lambda: self.onCommandClicked(
            Command(CommandMode.PAINT_SELECT_REMOVE, True), widget))
        Util.setModernTooltipText(paintSelectCommand, paintSelectCommandLabel)

        dropperCommandLabel: str = '吸管'
        dropperCommand: TransparentToggleToolButton = TransparentToggleToolButton(
            widget)
        dropperCommand.setIcon(FluentIconExt.EYEDROPPER)
        dropperCommand.clicked.connect(lambda: self.onCommandClicked(
            Command(CommandMode.DROPPER), widget))
        Util.setModernTooltipText(dropperCommand, dropperCommandLabel)

        paintCommandLabel: str = '画笔'
        paintCommand: TransparentToggleToolButton = TransparentToggleToolButton(
            widget)
        paintCommand.setIcon(FluentIconExt.INKING_TOOL)
        paintCommand.clicked.connect(
            lambda: self.onCommandClicked(Command(CommandMode.DRAW), widget))
        Util.setModernTooltipText(paintCommand, paintCommandLabel)

        fillColorCommandLabel: str = '油漆桶'
        fillColorCommand: TransparentToggleToolButton = TransparentToggleToolButton(
            widget)
        fillColorCommand.setIcon(FluentIconExt.PAINT_BUCKET)
        fillColorCommand.clicked.connect(lambda: self.onCommandClicked(
            Command(CommandMode.PAINT_BUCKET, True), widget))
        ...
        cropCommand: TransparentToggleToolButton = TransparentToggleToolButton(
            widget)
        cropCommand.setIcon(FluentIconExt.CROP)
        cropCommand.clicked.connect(lambda: self.onCommandClicked(
            Command(CommandMode.CROP), widget))
    ...
    def createTopMiddleCmdBar(self) -> QWidget:
        ...
        colorOverlayCommand.clicked.connect(lambda: self.onQuickActionClicked(
            QuickAction(QuickActionMode.COLOR_OVERLAY)))
        ...
        removeBgCommand.clicked.connect(lambda: self.onQuickActionClicked(
            QuickAction(QuickActionMode.REMOVE_BACKGROUND, True)))
        ...
        purifyBgCommand.clicked.connect(lambda: self.onQuickActionClicked(
            QuickAction(QuickActionMode.PURIFY_BACKGROUND, True)))
        ...
        colorizeCommand.clicked.connect(lambda: self.onQuickActionClicked(
            QuickAction(QuickActionMode.BRUSH, True)))
        ...
        rotateCommand.clicked.connect(lambda: self.onQuickActionClicked(
            QuickAction(QuickActionMode.ROTATE)))
        ...
        self.undoCommand.clicked.connect(
            lambda: self.onQuickActionClicked(QuickAction(QuickActionMode.UNDO)))
        ...
        self.redoCommand.clicked.connect(
            lambda: self.onQuickActionClicked(QuickAction(QuickActionMode.REDO)))
    ...
    def createBottomLeftCmdBar(self) -> QWidget:
        ...
        submitBtn.clicked.connect(lambda: self.onQuickActionClicked(
            QuickAction(QuickActionMode.TEXT, True), lineEdit.text()))
    ...
    def createBottomRightCmdBar(self) -> QWidget:
        ...
        zoomOutCommand.clicked.connect(lambda: self.onQuickActionClicked(
            QuickAction(QuickActionMode.ZOOM_OUT)))
        ...
        zoomInCommand.clicked.connect(lambda: self.onQuickActionClicked(
            QuickAction(QuickActionMode.ZOOM_IN)))
    ...
    def createColorWidget(self, color: QColor) -> QWidget:
        ...
        self.colorNameLabel: BodyLabel = BodyLabel(
            text=f'{color.name().upper()}')
        ...
        for child in parent.children():
            if isinstance(child, TransparentToggleToolButton):
                if child == self.sender():
                    self.imageWidget.onCommandClicked(
                        command if child.isChecked() else Command(CommandMode.UNKNOWN))
                else:
    ...
    def setZoomFactorLabelText(self):
        self.zoomFactorBtn.setText(
            self.tr("{:.0f}%".format(self.imageWidget.zoomFactor * 100)))
